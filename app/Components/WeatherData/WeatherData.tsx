...
const WeatherData = () => {
  ...
  return (
    <div
      className={`rounded-xl flex top-0 right-0 absolute w-[65%]  
      ${!toggle ? "translate-x-[100%]" : "translate-x-0"}
       h-screen bg-[#FFFFFF7F] duration-500 items-center backdrop-blur-sm ease-in-out`}>
      <button
        onClick={clickHandler}
        className={`h-10 w-10 text-black hover:bg-black hover:text-white rounded-[50%] text-5xl flex items-center absolute -left-[3vw] ${toggle && "rotate-180" } duration-300`}>
        <MdOutlineKeyboardArrowLeft />
      </button>
      <div className={"flex flex-col flex-1 text-black items-center h-svh p-4 gap-2"}>
        <DataGraph/>
        <DataCardGrid/>
      </div>
    </div>
